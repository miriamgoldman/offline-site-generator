...
define( 'OFFLINESITEGENERATOR_REDIRECT_CODES', [ 301, 302, 303, 307, 308 ] );
...
class Crawler {
    ...
    public function __construct() {
        ...
        $port_override = apply_filters(
            'offlineSiteGenerator_curl_port',
            null
        );
        ...
        curl_setopt(
            $this->ch,
            CURLOPT_USERAGENT,
            apply_filters( 'offlineSiteGenerator_curl_user_agent', 'OfflineSiteGenerator.com' )
        );
    ...
    public function crawlSite( string $static_site_path ) : void {
        ...
        $use_crawl_cache = apply_filters(
            'offlineSiteGenerator_use_crawl_cache',
        ...
        foreach ( CrawlQueue::getCrawlablePaths() as $root_relative_path ) {
            ...
            if ( in_array( $response['code'], OFFLINESITEGENERATOR_REDIRECT_CODES ) ) {
            ...
            if ( $crawled_contents ) {
                ...
                if ( mb_substr( $root_relative_path, -1 ) === '/' ) {
                    ...
                    $processed_url = $root_relative_path . 'index.html';
                } else {
                    StaticSite::add( $root_relative_path, $crawled_contents );
                    $processed_url = $root_relative_path;
                }

            }

            CrawlCache::addUrl(
                substr($processed_url, 1),
                $page_hash,
                $response['code'],
                $redirect_to
            );
        ...
        do_action( 'offlineSiteGenerator_crawling_complete', $args );
    }
    ...
    public function crawlURL( string $url ) : ?array {
        ...
        if ( $response['code'] === 404 ) {
            ...
            $response['body'] = null;
        } elseif ( in_array( $response['code'], OFFLINESITEGENERATOR_REDIRECT_CODES ) ) {
            $response['body'] = null;
        }
